...
type ComplianceSummary = { framework: string; deadline: string; ready_percent: number; ready_assets: number; total_assets: number; explanation: string };
...
const baseHours = { critical: 16, high: 8, medium: 4, low: 2 } satisfies Record<Level, number>;
const agilityFor = (asset: Asset, assets: Asset[]) => asset.crypto_agility_score ?? Math.max(0, 30 - Math.max(0, assets.filter((candidate) => candidate.algorithm === asset.algorithm).length - 1) * 5) + (/(crypto|security|encryption)/i.test(asset.location) ? 15 : 0);
const effortFor = (asset: Asset, assets: Asset[], hours: Record<Level, number>) => {
  const agility = agilityFor(asset, assets);
  const estimatedHours = Math.round(hours[asset.risk_level] * (0.75 + 1.25 * (100 - agility) / 100) * 10) / 10;
  return { agility, hours: estimatedHours };
};
...
function App() {
  ...
  const [sortBy, setSortBy] = useState<"risk" | "agility">("risk");
  ...
  const [severityHours, setSeverityHours] = useState<Record<Level, number>>(baseHours);
  ...
  const visible = assets.filter((asset) =>
    ...
    : asset
  ).sort((left, right) => sortBy === "agility" ? agilityFor(left, assets) - agilityFor(right, assets) : right.risk_score - left.risk_score);
  const topPriority = assets.find((asset) => asset.risk_level === "critical") ?? assets[0];
  const totalEffort = useMemo(() => assets.reduce((total, asset) => total + effortFor(asset, assets, severityHours).hours, 0), [assets, severityHours]);
  ...
  async function loadAssessmentExtensions(id = scanId) {
    ...
    try {
      const [matrixResponse, complianceResponse] = await Promise.all([fetch(`${apiUrl}/api/v1/scans/${id}/exposure-matrix`), fetch(`${apiUrl}/api/v1/scans/${id}/compliance/cnsa-2.0`)]);
      if (!matrixResponse.ok || !complianceResponse.ok) throw new Error("Assessment insights are unavailable");
      setMatrix(await matrixResponse.json() as ExposureMatrixItem[]);
      setCompliance(await complianceResponse.json() as ComplianceSummary);
    } catch (error) { setNotice(error instanceof Error ? error.message : "Assessment insights failed"); } finally { setLoadingAssessment(false); }
  }
  ...
  return <main className={`${scanId ? "has-scan" : "awaiting-scan"} ${workspace === "workflow" ? "workflow-active" : ""} ${workspace === "discovery" ? "discovery-active" : ""} ${workspace === "prioritize" ? "priority-active" : ""}`}>
    ...
    {workspace === "overview" && <section className="assessment-insights"><div className="insight-heading"><div><p className="eyebrow">Exposure and compliance</p><h2>Where quantum risk meets external exposure</h2><small>Each marker is a finding. Position reflects the parsed entry-point exposure and calculated severity.</small></div>{scanId && <button className="impact-button" onClick={() => void loadAssessmentExtensions()} disabled={loadingAssessment}>{loadingAssessment ? "Refreshing..." : "Refresh insights"}</button>}</div>{compliance && <div className="compliance-stat"><b>{compliance.ready_percent}%</b><div><strong>CNSA 2.0 planning readiness</strong><small>{compliance.ready_assets} of {compliance.total_assets} findings meet the readiness indicator for the {compliance.deadline}.</small></div></div>}{scanId ? <div className="exposure-matrix"><div className="matrix-label top">Higher severity</div><div className="matrix-label left">Internal</div><div className="matrix-grid"><div><b>Prioritize internally</b></div><div><b>Immediate external risk</b></div><div><b>Monitor</b></div><div><b>Manage externally</b></div>{matrix.map((item, index) => <button key={item.asset_id} className={`matrix-dot ${item.risk_level} ${item.exposure === "public-facing" ? "external" : "internal"} ${item.hndl_candidate ? "hndl" : ""}`} style={{ left: `${item.exposure === "public-facing" ? 58 + (index % 4) * 9 : 8 + (index % 4) * 9}%`, top: `${item.risk_score >= 60 ? 12 + (index % 4) * 8 : 58 + (index % 4) * 8}%` }} title={`${item.algorithm}: ${item.exposure}; HNDL score ${item.hndl_score}/100`} onClick={() => { const asset = assets.find((candidate) => candidate.asset_id === item.asset_id); if (asset) setSelected(asset); }}><span>{item.hndl_candidate ? "HNDL" : item.algorithm}</span></button>)}</div><div className="matrix-label bottom">Internet-facing</div><p>{matrix.filter((item) => item.hndl_candidate).length} HNDL candidate(s): quantum-vulnerable findings with an at-risk Mosca window and high-criticality data context.</p></div> : <p className="empty-state">Run an assessment to calculate the exposure matrix and CNSA 2.0 planning indicator.</p>}<section className="remediation-preview"><p className="eyebrow">Illustrative remediation preview</p><h2>{selected.algorithm} migration pattern</h2><small>Example only: validate library availability, protocol compatibility, and tests before making a source change.</small><div><pre><b>Before</b>{remediationPreview(selected.algorithm).before}</pre><pre><b>After</b>{remediationPreview(selected.algorithm).after}</pre></div></section></section>}
    ...
    {workspace !== "migration" && <section className="finding-extensions"><div><p className="eyebrow">Planning effort</p><h2>Crypto-agility and local planning inputs</h2><small>Agility measures ease of change, not security risk. Hours are a local planning indicator.</small></div><dl><div><dt>Crypto-Agility Score</dt><dd>{agilityFor(selected, assets)}/100<small>{selected.crypto_agility_reason ?? "Demo estimate based on reuse and module placement."}</small></dd></div><div><dt>Planning effort indicator</dt><dd>{effortFor(selected, assets, severityHours).hours.toFixed(1)} hours<small>Severity base hours adjusted by the agility score; validate the actual scope before delivery planning.</small></dd></div></dl><div className="evidence-sort"><label htmlFor="finding-sort">Evidence order</label><select id="finding-sort" value={sortBy} onChange={(event) => setSortBy(event.target.value as "risk" | "agility")}><option value="risk">Highest risk score</option><option value="agility">Lowest crypto-agility score</option></select><small>Sort findings by urgency or migration difficulty.</small></div></section>}
    {workspace === "migration" && <section className="cost-settings"><div><p className="eyebrow">Effort assumptions</p><h2>Prototype planning inputs</h2><small>Formula: severity base hours x (0.75 + 1.25 x (100 - agility) / 100). These values are local planning inputs, not estimates of delivery cost.</small></div><div className="cost-inputs">{riskOrder.map((level) => <label key={level}>{level} base hours <input type="number" min="0" value={severityHours[level]} onChange={(event) => setSeverityHours((current) => ({ ...current, [level]: Number(event.target.value) }))} /></label>)}</div><div className="cost-total"><b>{totalEffort.toFixed(1)} hours</b><span>total prototype effort indicator</span><small>Across {assets.length} findings using the current local assumptions.</small></div></section>}
  </main>;
}
